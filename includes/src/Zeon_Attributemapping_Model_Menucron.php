        ...
        try{
            if ($manuType == 'category') {
                ...
                foreach ($options as $optData) {
                    ...
                    //For Categories with zero products
                    $storeId = Mage::app()->getStore()->getStoreId();
                    $resource_procount = Mage::getSingleton('core/resource');
                    $readConnection_procount = $resource_procount->getConnection('core_read');
                    $query_procount = $readConnection_procount->select()->from('catalog_category_product_index', array('count(*) as procount'))
                             ->where('category_id=?',$optData['option_id'])
                             ->where('store_id=?',$storeId);
                    $results_procount = $readConnection_procount->fetchAll($query_procount);
                    $procount = $results_procount[0]['procount'];
