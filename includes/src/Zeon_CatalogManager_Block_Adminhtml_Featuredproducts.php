 ...
 */
class Zeon_CatalogManager_Block_Adminhtml_Featuredproducts
    extends Mage_Adminhtml_Block_Widget_Grid_Container
{
    ...
    public function __construct()
    {
        ...
        $this->_headerText =$helper->__('Featured products');

        $this->_removeButton('add');
        $this->_addButton(
            'save',
            array(
                'label'   => $helper->__('Save Featured Products'),
    ...
    {
        $html = '
    	<form id="featured_edit_form" action="' . $this->getSaveUrl() . '"
            method="post" enctype="multipart/form-data">
    	<input name="form_key" type="hidden"
            value="' . $this->getFormKey() . '" />
    		<div class="no-display">
        		<input type="hidden" name="featured_products" id="in_featured_products" value="" />
                <input type="hidden" name="featured_products_position" id="featured_products_position" value="" />
    		</div>
		</form>
    	';
