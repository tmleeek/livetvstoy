    ...
    foreach ($remote_files as $remote_file) {
        ...
        if (($inputfp = fopen($local_file, "r")) !== FALSE) {
            ...
            while (($data = fgetcsv($inputfp, 1000, ",")) !== FALSE) {
                ...
                if($i != 0) {
		    if ($data[0] == ''){
			print "Error : Some of the tokens are missing in the file. Please correct the input file and rerun the script.\n\n ";
			...
			$recipient = "bpunati@cpscompany.com";
		        $mail_body = "Error : Some of the tokens are missing in the file. Please correct the input file and rerun the script";
		        $subject = "Walmart-NS Sync Alert!"; //subject
			...
			//exit;
                    ...
                    $query = "SELECT slcp.product_id,eaov.value, slewt.raw_pers_string FROM site_link_external_walmart_transactions AS slewt LEFT JOIN site_link_configure_products AS slcp ON slewt.cps_sku = slcp.cps_sku LEFT JOIN eav_attribute_option_value AS eaov ON slewt.extpartno = eaov.option_id WHERE slewt.token_id = $token_id";
