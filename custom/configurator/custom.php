	...
	function FUNC_CUSTOM_cbo_ready($arr, $type, $def = NULL) {
		...
		foreach($arr as $a => $b) {
			...
			// For Metals	
				...
				if(isset($b['is_required']))
					$ret_arr[$a]['is_required'] = $b['is_required'];
...
if (!function_exists('FUNC_CUSTOM_stones')) {
	function FUNC_CUSTOM_stones($pid, $type, $def = NULL, $label = NULL, $list) {
		...
		foreach ($product->getOptions() as $o) {
			if($o->getEnable() == 'true') {
				...
				//if ($o->getType() == 'drop_down' && ($o->getClassCode() == 'BIRTH_STONE' || $o->getClassCode() == 'BIRTH_STONE1' || $o->getClassCode() == 'BIRTH_STONE2')) {
					...
					$options = array();
					$options['id'] = $o->getId();
					$options['title'] = $o->getTitle();
					$options['type'] = $o->getType();
					$options['code'] = $o->getClassCode();
					$options['is_required'] = $o->getIsRequire();
...
if (!function_exists('FUNC_CUSTOM_sizes')) {
	function FUNC_CUSTOM_sizes($pid, $type, $def = NULL) {
		...
		$productId = $product->getIdBySku( $pid );
		$product->load($productId);
		$productType = $product->getTypeId(); // detect product type
		...
		if ( $productType == 'configurable' ){
			...
			if ( $productAttributeOptions ){
				...
				foreach ($allProducts as $a) {
					if ($a->getIsInStock() == 1) { //in stock for child simple product
						$b = array();
						$b['size'] = $a->getSize();
						$sizes[] = $b;
					}
				...
				foreach ($productAttributeOptions as $o ) {
					if ($o['label'] == "Size" || $o['label'] == "Birthstone") {
						
						foreach ($o['values'] as $k => $v) {
							
							foreach ($sizes as $s) {
								if ($s['size'] == $v['value_index']) {
				...
				//echo "<pre>";
				//print_r($internal_arr['stones']);
				//echo "</pre>";
				
				return $internal_arr['stones'];
		}*/
...
if (!function_exists('FUNC_CUSTOM_ddstones')) {
	function FUNC_CUSTOM_ddstones($pid, $type, $def = NULL) {
