		...
		foreach( $scripts as $type => $val ){
			...
			if ( $type == 'css'){
				foreach($val as $script){
					echo '<link rel="stylesheet" type="text/css" href="' . $base_url . '/configurator/interface/' . $script .'">';
				}
			}
			elseif ( $type == 'js' ){
				foreach($val as $script){
					echo '<script type="text/javascript" src="' . $base_url . '/configurator/interface/' . $script .'"></script>';
				}
			}
		}
	}
...
if (!function_exists('FUNC_get_XML')) {
	function FUNC_get_XML($dir, $value=null, $assoc_layer=null, $assoc_step = NULL, $comp_sel_id = NULL) {
		...
		if($dir == 'layer') {
			...
			include  __DIR__."/products/".$product_template."/layers.php";
			
		}else if($dir == 'dd_stones') {
			
			include  __DIR__."/products/".$product_template."/dd_stones.php";
			...
			include  __DIR__."/products/".$product_template."/fields.php";
			...
			include  __DIR__."/components/metals.php";
			
		}else if($dir == 'stones') {
			// Path to Dynamic Stones XML
			//include("products/".$product_template."/stones.xml");
			include  __DIR__."/components/stones.php";
		...
		if($dir == 'custom') {
			...
			if(isset($pre_arr) and is_array($pre_arr) and count($pre_arr) > 0){
				...
				if($dir == 'fields' || $dir == 'layer' || $dir == 'metal' || $dir == 'stones' || $dir == 'dd_stones') {
...
if (!function_exists('get_selected_base')) {
	function get_selected_base($arr){
	
		$selected = $arr[0]['ddstones'];
	
		$internal_arr = FUNC_get_XML('dd_stones');
		
		foreach( $internal_arr['stones'] as $idx => $val ){
		...
		$sel_dd['selected_dd']['base'] = $base;
		$sel_dd['selected_dd']['rules'] = $rules;
		$sel_dd['selected_dd']['value'] = $value;
		/*
		echo '<pre>'; 
...
if (!function_exists('restructureArr')) {
	function restructureArr($arr){
		...
		foreach( $internal_arr['stones'] as $idx => $val ){
			...
			if( $val['value'] == $selected ){
				$rules = $val['rules'];
				$textrules = $val['textrules'];
				$textindex = isset($val['textindex']) ? $val['textindex'] : 0;
			}
		}
	/*
		echo '<pre>'; 
	...
	*/
		...
		$arg = clean_arrvalues($arr);
		
		$rules = explode('|', $rules);
		$textrules = explode('|', $textrules);
		/*
		echo "<pre>";
		print_r($rules);
		print_r($stones);
		...
		*/
		foreach( $rules as $idx => $val ){
			$arg[$val]['stonesd'] = $stonesd[$idx];
			//$arg[$val + count($stonesd)]['text'] = $text[$idx];
		}
		
		foreach( $textrules as $idx => $val ){
			$arg[$val]['textd'] = $textd[$idx];
			//$arg[$val + count($stonesd)]['text'] = $text[$idx];
		}
...
if (!function_exists('clean_arrvalues')) { 
	function clean_arrvalues($arr){
