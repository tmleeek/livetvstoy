...
try {
	if($sku){
	 ...
	 $results_or= $writeConnection->fetchAll($query_order);
		...
		echo $itemId = $results_or[0]['item_id'];
	 }
...
//	 $orderItem ->setProductOptions($serialize_array)->save();
	 $query_update = "UPDATE sales_flat_order_item SET product_options='".$product_options."', product_options_json='".$product_json."' WHERE order_id=".$magento_order_id." AND sku=".$sku;
