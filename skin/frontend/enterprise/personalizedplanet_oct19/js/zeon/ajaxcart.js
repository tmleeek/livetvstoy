                        ...
                        onSuccess : function(response) {
                            if (response && response.responseText) {
                                ...
                                if(result.empty=="empty") {
                                    document.getElementById('cartUpdateMsg').innerHTML='Shopping Cart Is Empty';
                                }

                                if (result.message && result.empty!="empty") {
                                    $('message-content').update(result.message);
                                }
                                // Update the header
                                if (typeof (result.header) != 'undefined') {
    ...
    addUpdate : function(postUrl, thisObj) {
        ...
        if (allsetType) {
            ...
            jQuery(personalizeFieldId).each(function(  index, value  ) {
                ...
                if (personalizeFieldValue.toLowerCase() == 'optional') {
                    jQuery(value).val('');
                }
            });
