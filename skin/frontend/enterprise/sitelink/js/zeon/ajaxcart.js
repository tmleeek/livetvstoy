...
AjaxCart.prototype = {
    ...
    closeMessagePopup : function() {
		...
		if (url=='') {
			window.location = "http://www-e16.walmart.com/cart";
		}else{
                        ...
                        onSuccess : function(response) {
                            if (response && response.responseText) {
                                ...
                                this.showButtons(loginRequired, isWishlist,
                                        isCompare);
                                this
                                        .openMessagePopup(AJAX_CART.isAutoHidePopup);
                            }
                        }.bind(this)
                    });
        } else {
    ...
    addUpdate : function(postUrl, thisObj) {
		postUrl = postUrl.replace("https://", "http://");
        postUrl = postUrl.replace("secure.", "");
		var j_add_to_cart = jQuery(".js-add-to-cart");
		j_add_to_cart.attr('disabled', true);
		if(j_add_to_cart.find(".btn-progress-spinner").length === 0){
			j_add_to_cart.append("<b class='btn-progress-spinner'><b class='btn-progress-spinner-icon'></b></b>");
		}
		j_add_to_cart.addClass('btn-progress');
        var isWishlist = false;
        var allsetType = true;
        if (postUrl.include('wishlist')) {
            isWishlist = true;
        ...
        if (!productForm.validator.validate() && !isWishlist) {
			//var progressBtn = new ProgressBtn(); progressBtn.deactivate(jQuery(".js-add-to-cart"));
			j_add_to_cart.attr('disabled', false);
			j_add_to_cart.removeClass('btn-progress');
            return false;
        }
        ...
        if (allsetType) {
			...
			var split = result.message.split('~');
			    ...
			    this.redirect_url = split[1];
                        ...
                        this.showButtons(loginRequired, isWishlist, false);
                        //this.openMessagePopup(3);
						var url=this.redirect_url;
						if (url=='') {
							window.location = "https://www.walmart.com/cart";
						}else{
