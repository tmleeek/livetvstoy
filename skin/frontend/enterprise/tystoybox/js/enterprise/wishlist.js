...
Enterprise.Wishlist.Widget.Form = Class.create(Enterprise.Widget, {
    ...
    initialize: function($super, action) {
        ...
        this.action = action;
        $super(new Element('form', {'method': 'post', 'action': action, 'id':'create-wishlistForm'}));
        ...
        var deferredList = {
            ...
            success: function() {
                ...
                if ($('product_addtocart_form') || $('product_updatecart_form')) {
                    this.counter = 0;
                }
            }
        };
...
Enterprise.Wishlist.createWithCallback = function(createUrl, callback, useAjax) {
    ...
    //createUrl = createUrl.replace("secure.", "");
    ...
    /*** Popup clear loding ***/
    if(jQuery('.popup-block').hasClass('loading')) {
...
Enterprise.Wishlist.copyItemTo = function(itemId, qty, wishlistId) {
    if(Enterprise.Wishlist.url.EnableAjax) {
        ajaxCart.ajaxCartWishlistMoveCopy(itemId, qty, wishlistId, 'copy');
    } else {
...
Enterprise.Wishlist.moveItemTo = function(itemId, qty, wishlistId) {
    if(Enterprise.Wishlist.url.EnableAjax) {
        ajaxCart.ajaxCartWishlistMoveCopy(itemId, qty, wishlistId, 'move');
    } else {
...
Enterprise.Wishlist.copyItemToNew = function(itemId, qty) {
    Enterprise.Wishlist.url.EnableAjax = false;
...
Enterprise.Wishlist.ShowDropdown = function() {
    try {
