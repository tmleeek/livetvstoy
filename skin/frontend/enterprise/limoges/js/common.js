function toggleSlider() {
    jQuery(document).on('click', '.slide-icon', function () {
        if (jQuery(this).next(".slide-toggle").is(':hidden')) {
			...
			var deviceName2 = isMobile() ? "android" : "desktop";
            if (deviceName == 'iphone' || deviceName2 == 'android') {
...
//on account icon hover open
if (!isTouch()) {
	jQuery('.user-img.slide-icon,.quick-access.slide-toggle').hover(function () {
		jQuery('.user-img.slide-icon').click(function(e) {
            return false;
        });;
		jQuery('.user-img.slide-icon').next(".quick-access.slide-toggle").stop().slideDown();
		jQuery('.user-img.slide-icon').addClass('t-expand');
		if(jQuery(".top-cart .block-content").is(':visible')){
			Enterprise.TopCart.hideCart();
		}
	});
	jQuery('.user-img.slide-icon,.quick-access.slide-toggle').mouseleave(function () {
		jQuery('.user-img.slide-icon').next(".quick-access.slide-toggle").stop().slideUp();
		jQuery('.user-img.slide-icon').removeClass('t-expand');
	});
	...
	jQuery('.top-cart').hover(function () {
	    ...
	    jQuery('#topCartContent').stop().slideDown();
	    jQuery('#topCartContent').addClass('t-expand');
	    jQuery('.top-cart').mouseleave(function () {
		jQuery('#topCartContent').stop().slideUp();
		jQuery('#topCartContent').removeClass('t-expand');
	    });
	});
