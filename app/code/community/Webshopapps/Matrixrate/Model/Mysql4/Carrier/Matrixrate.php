...
class Webshopapps_Matrixrate_Model_Mysql4_Carrier_Matrixrate extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    public function getNewRate(Mage_Shipping_Model_Rate_Request $request,$zipRangeSet=0)
    {
	    ...
	    $quoteData   = $cart1->getData();
	    $couponCode=$quoteData['coupon_code'];
	    $oCoupon = Mage::getModel('salesrule/coupon')->load($couponCode, 'code');
		//print_r($oCoupon);
	    $checkout = Mage::getSingleton('checkout/session')->getQuote();
	    $getShippingAddress = $checkout->getShippingAddress()->getData('country_id');
	    //var_dump($getShippingAddress);
	    $wid=$request->getWebsiteId();
	    $query_sel = "SELECT distinct(`rule_id`) FROM `flat_shipping_matrixrate` where dest_country_id='$getShippingAddress' and website_id=$wid";
            $results = $writeConnection->fetchAll($query_sel);
	    $rule_array=array();
	    foreach($results as $key=>$value) {
		$rule_array[]= $value['rule_id'];
	    }
	    if (in_array($oCoupon['rule_id'], $rule_array))
	    $table = 'flat_shipping_matrixrate';
	    else
		...
		for ($j=0;$j<10;$j++)
		{
			...
			if (in_array($oCoupon['rule_id'], $rule_array))
			$select->where('rule_id=?', $oCoupon['rule_id']);
...
//            }
		    ...
		    //
                     $new_shipment = "There are new shipping methods added to Magento</br></br><table><tr><th>pk </th><th> dest_country_id </th><th> price </th><th> delivery_type</th></tr>";
			   ...
			   //
                           $new_shipment .= "<tr><td>".$connection->lastInsertId()." </td><td> ".$dataLine['dest_country_id']." </td><td> ".$dataLine['price']." </td><td> ".$dataLine['delivery_type']."</td></tr>";
		    ...
		    //Send mail notificaion on Insert
		    $mail = Mage::getModel('core/email');
                    $mail->setToName('Magento Alert');
                    $mail->setToEmail('magentoalerts@cpscompany.com');
                    $mail->setBody($new_shipment."</table>");
                    $mail->setSubject('Shipping Method Notification ');
                    $mail->setFromEmail('magento@cps.com');
                    $mail->setFromName("Magento");
