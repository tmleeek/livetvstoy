...
class Celigo_Celigoconnector_Model_Asyncconnection extends Mage_Core_Model_Abstract {
    ...
    public function makeAsyncCall($url, $params) {
        if (!function_exists("fsockopen")) {
            $this->logger->error( 'errormsg="'."fsockopen function doesn't exist".'"', self::LOG_FILENAME);
        ...
        while ($isAllowed) {
            ...
            try {
                ...
                if (!$fp) {
                    ...
                    $this->logger->error( 'errormsg="'."fsockopen {$errno}: {$errstr}".'"', self::LOG_FILENAME);
                } else {
                ...
                if ($numOfMaxTries > 0) {
                 ...
                 $this->logger->error( 'errormsg="makeAsyncCall::Exception '.$e->getMeesage().'"', self::LOG_FILENAME);
            }
        }
    ...
    public function makePersistantAsyncCall($url, $params) {
        if (!function_exists("pfsockopen")) {
             $this->logger->error( 'errormsg="'."pfsockopen function doesn't exist".'"', self::LOG_FILENAME);
        ...
        while ($isAllowed) {
            ...
            try {
                ...
                if (!$fp) {
                    ...
                    $this->logger->error( 'errormsg="'."pfsockopen {$errno}: {$errstr}".'"', self::LOG_FILENAME);
                } else {
                    $out = "POST " . $parts['path'] . " HTTP/1.1\r\n";
                    $out.= "Host: " . $host . "\r\n";
                    $out.= "Content-Type: application/x-www-form-urlencoded\r\n";
    ...
    public function makeAsyncCurlCall($url, $params) {
        try {
            ...
            if ($info->http_code != 200) {
                $this->logger->error( 'errormsg="makeAsyncCurlCall::Error '.$error.'"', self::LOG_FILENAME);
                return false;
            }
            if (isset($params['orderId'])) {
                Mage::getSingleton('core/session')->setPaymentDetails('');
            ...
            $this->logger->error( 'errormsg="'."makeAsyncCall::Exception". $e->getMessage() .'"', self::LOG_FILENAME);
