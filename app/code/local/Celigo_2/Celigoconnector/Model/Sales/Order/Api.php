...
//Mage_Api_Model_Resource_Abstract
{
    ...
    protected function _initOrder($orderIncrementId) {
        ...
        if (!$order->getId()) {
            $this->logger->error('errormsg="No order found with increment id "'.$orderIncrementId.'", record="order" api="_initOrder"', self::LOG_FILENAME);
    ...
    public function update($orderIncrementId, $data) {
        if (!is_array($data)) {
            $this->logger->error('errormsg="Order data must be an array." recordid="'.$orderIncrementId.'", record="order" api="order.update"', self::LOG_FILENAME);
        ...
        try {
            ...
            if (trim($pushedOrderStatus) != "" && $isCancelCallBack == false) {
            ...
            $this->logger->error('errormsg="'.$e->getMessage().'", record="order" api="order.update"', self::LOG_FILENAME);
