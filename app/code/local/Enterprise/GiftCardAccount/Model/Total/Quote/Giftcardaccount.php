...
class Enterprise_GiftCardAccount_Model_Total_Quote_GiftCardAccount extends Mage_Sales_Model_Quote_Address_Total_Abstract
{
    ...
    public function collect(Mage_Sales_Model_Quote_Address $address)
    {
        ...
        $baseTotalUsed = $totalUsed = $baseUsed = $used = $skipped = $baseSaved = $saved =  $check = 0;
        $baseTotalUsedForGc = 1;
        $deductDonationAmtFromDiscount = 0;
        $containDonation = false;
        ...
        if ($baseAmountLeft >= $address->getBaseGrandTotal()) {
            ...
            $baseTotalUsedForGc = $address->getGrandTotal()?$address->getGrandTotal():$baseTotalUsedForGc;
            $check = $address->getGrandTotal()?$address->getGrandTotal():0;
            $quoteItems = $address->getQuote()->getAllItems();
            if ($donationSku) {
                foreach ($quoteItems as $item) {
                    if ($item->getProduct()->getSku() == $donationSku) {
                        $containDonation = true;
                        break;
                    }

                }
            }
            if ($containDonation && $check) {
                $address->setBaseGrandTotal($address->getBaseGrandTotal()/$address->getBaseGrandTotal());
                $address->setGrandTotal($address->getBaseGrandTotal()/$address->getBaseGrandTotal());
                $deductDonationAmtFromDiscount = 1;
            } else {
        ...
        if ($baseUsed) {
            $quoteCards = $this->_sortGiftCards(
                Mage::helper('enterprise_giftcardaccount')->getCards($address->getQuote())
            );
            foreach ($quoteCards as $quoteCard) {
                ...
                } elseif ($quoteCard['ba'] + $skipped + $baseSaved >
                    $address->getQuote()->getBaseGiftCardsAmountUsed()) {
        ...
        if ($containDonation) {
            $baseTotalUsed = $baseTotalUsed - $deductDonationAmtFromDiscount;
        }
        $totalUsed = $address->getQuote()->getGiftCardsAmountUsed() + $used ;

        if ($containDonation ) {
            $totalUsed = $totalUsed - $deductDonationAmtFromDiscount;
        }
        $address->getQuote()->setBaseGiftCardsAmountUsed($baseTotalUsed);
        $address->getQuote()->setGiftCardsAmountUsed($totalUsed);
        if ($containDonation) {
            $baseUsed = $baseUsed - $deductDonationAmtFromDiscount;
            $used = $used - $deductDonationAmtFromDiscount;
        }
    ...
    public function fetch(Mage_Sales_Model_Quote_Address $address)
    {
        if ($address->getQuote()->isVirtual()) {
            $giftCards = Mage::helper('enterprise_giftcardaccount')->getCards(
