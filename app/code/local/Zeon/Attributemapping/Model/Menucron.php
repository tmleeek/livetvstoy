...
class Zeon_Attributemapping_Model_Menucron extends Mage_Core_Model_Abstract
{
        ...
        try{
            ...
            foreach ($catChilds as $childs) {
                ...
                if (count($childSubcat) > 0) {
                    ...
                    /* Added code if the category has not subcategories */
                    $categoryData[$childs->getId()] = array(
                        'name' => $childs->getName(),
                        'url'  => $childs->getUrl(),
                    );
    ...
    protected function _getSingleCategoryMenuCollection($store, $catId, $code = 'personalized_gifts')
    {
        ...
        try{
            ...
            if ($code == 'personalized_gifts')
                $categoryData['url'] = '{{store url}}personalized-gifts';
            $catData['menuno'] = 1;
        ...
        try{
            if ($manuType == 'category') {
                ...
                foreach ($categoryData as $cat) {
                    ...
                    $output .= '<span class="sub-menu-title"><a href="'.$cat['url'].'">'
                        .$cat['name'].'</a></span>';
                ...
                $output .= '<a class="level-top" title="'.$title.'"'
                    . ' setshow="'.$categoryData['code'].'" href="{{store url}}all-character">'
                    .'<span>'.$title.'</span></a>';
                ...
                foreach ($options as $optData) {
                    if($categoryData['code'] == 'brand' && $store == 2) {
