...
class Zeon_CatalogManager_Model_Bestseller
{
    ...
    protected function _updateProductTopSelling($attribute, $storeId = '0')
    {
        try {
            ...
            LEFT JOIN `catalog_product_entity_varchar` AS `cper` ON cper.entity_id = order_items.product_id 
            AND cper.attribute_id = 281 
            ...
            foreach ($orderItemsquery as $_item) {
                ...
                if ($_item['ordered_qty'] > 0) {
                    ...
                    if ($totalUpdateCount == 0 )
                        $max_rate = (int) $_item['ordered_qty'];

                    $sell_rank = (int) $_item['sell_rank'];

                    if ($sell_rank > 0)
                        $itemQty = $_item['ordered_qty'] + $max_rate - $sell_rank;
                    else 
