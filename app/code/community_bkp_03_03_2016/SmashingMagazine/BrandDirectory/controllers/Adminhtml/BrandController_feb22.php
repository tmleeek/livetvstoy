    ...
    if (($handle_file = fopen($file_path, "r")) !== FALSE)
        {
            ...
            if (!$product){
             ...
             $resource = Mage::getSingleton('core/resource');
            $writeConnection = $resource->getConnection('core_write');
            $query_sel_dup = "select * from site_link_configure_products where part_no='$product_data[0]'";
    ...
    public function exportfileAction()
    {
     ...
     foreach($results as $simple_product){
        ...
        // echo "2_";
         ...
         {
         $_Pdetails = Mage::getModel('catalog/product')->loadByAttribute('sku',$simple_product['cps_sku']);
