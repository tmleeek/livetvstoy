    ...
    implements Mage_Shipping_Model_Carrier_Interface
{
    ...
    public function collectRates(Mage_Shipping_Model_Rate_Request $request)
    {
        ...
        if ($request->getAllItems()) {
            ...
            foreach ($request->getAllItems() as $item) {
                //end
                if ($item->getProduct()->isVirtual() || $item->getParentItem() || ($item->getProduct()->getSku() =='donate')) {
                    continue;
                }
                $itemCount++;
                ...
                if ($item->getProductType() == 'configurable') {
                    ...
                    $dropShipAttributValue = $productResource->getAttributeRawValue($productId, 'drop_ship_item', Mage::app()->getStore());
                    Mage::log('$dropShipAttributValue - ' .$dropShipAttributValue, Null, 'shipping.log', true);
                    //check for presence of drop ship attribute
                    if ($dropShipAttributValue) {
                        $dropShipItemCount++;
                        //Code Modified By Zeon
                        //check for bulk extra shipping cost
