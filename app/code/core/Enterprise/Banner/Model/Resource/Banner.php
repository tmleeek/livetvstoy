...
class Enterprise_Banner_Model_Resource_Banner extends Mage_Core_Model_Resource_Db_Abstract
{
    ...
    public function getExistingBannerIdsBySpecifiedIds($bannerIds, $isActive = true)
    {
        ...
        $select = $adapter->select()
            ->from($this->getMainTable(), array('banner_id','date_from','date_to'))
            ->where('banner_id IN (?)', $bannerIds);
	...
	$now_date_stamp = Mage::getModel('core/date')->timestamp(time());
        $bannerIdsVal = array();
        if ($data = $adapter->fetchAll($select)) {
            foreach ($data as $row) {
                if($row['date_from'] && $row['date_to']){
                    $date_from_stamp = Mage::getModel('core/date')->timestamp($row['date_from']);
                    $date_to_stamp = Mage::getModel('core/date')->timestamp($row['date_to']);
                    // $now_date_stamp;
                    if((int)$now_date_stamp >= (int)$date_from_stamp+21600 && (int)$now_date_stamp <= (int)$date_to_stamp+21600){
                        $bannerIdsVal[] = $row['banner_id'];
                    }
                }else{
                    $bannerIdsVal[] = $row['banner_id'];
                }
	...
	return array_intersect($bannerIds, $bannerIdsVal);
    }
