	...
	public function pushOrderToNS($orderId = '') 
	{
        ...
        try {
			...
			if (in_array($order->getStatus(), $status)) {
				...
				if($orderInfo['payment']["method"]=='authorizenet') {
                    foreach($orderInfo['payment']['additional_information']['authorize_cards'] as  $ccdetails) {
			...
			$orderInfo['payment']['cc_type']=$ccdetails['cc_type'];		
