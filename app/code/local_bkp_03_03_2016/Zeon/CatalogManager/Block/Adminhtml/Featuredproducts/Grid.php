 ...
 */
class Zeon_CatalogManager_Block_Adminhtml_Featuredproducts_Grid
    extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    protected function _prepareCollection()
    {
        ...
        $collection = Mage::getModel('catalog/product')->getCollection()
                ...
                //->addAttributeToSelect('featured_product_position')
        ...
        if ($store->getId()||$store->getId() == 0 ) {
            ...
            $collection->joinAttribute(
                ...
                'catalog_product/featured_product_position',
                'entity_id',
                null,
                'left',
                $store->getId()
            ...
            $collection->joinAttribute(
                'featured_product',
                'catalog_product/featured_product',
                'entity_id',
                null,
                'left',
                $store->getId()
        ...
        if ($actionName == 'exportCsv' || $actionName == 'exportXml') {
            $collection->addAttributeToFilter(
                'featured_product', array('eq' => true)
            );
        }
    ...
    protected function _prepareColumns()
    {
        ...
        if (!$this->_isExport) {
            $this->addColumn(
                ...
                array(
                    ...
                    'width'     => '1',
                    'type'      => 'number',
                    'index'     => 'featured_product_position',
        ...
        $this->addExportType(
            '*/*/exportCsv/type/featured',
            $helper->__('CSV')
        );
        $this->addExportType(
            '*/*/exportXml/type/featured',
            $helper->__('Excel XML')
        );
    ...
    {
        return $this->getUrl('*/*/grid', array('_current' => true,'blockname'=>'adminhtml_featuredproducts_grid'));
    }
    ...
    protected function _getSelectedProducts($json = false)
    {
        ...
        if ($json == true) {
            ...
            if (isset($featuredIds)) {
                foreach ($featuredIds as $key => $value) {
                    if ($value == 1) {
    ...
    function handleFeatureProductPositionBlur(event, element, trElement)
    {
        ...
        $("featured_products_position").value = textBoxes.toQueryString();

    }

    function isCheckboxChecked()
    {
        var everycheckbox = $$("#featured_products_table tbody input.checkbox");
    ...
    {
        var everytextbox = $$("#featured_products_table tbody input.input-text");
        everytextbox.each(function(element, index) {
            element.onkeyup = function(event)
            {
                var trElement = Event.findElement(event, 'tr');
                handleFeatureProductPositionBlur(event, element, trElement)
            }
        });
    }
    ...
    {
        var params = {};
        var fields = $('featured_edit_form').getElementsBySelector(
            'input', 'select'
        );
...
EndHTML;
    ...
    private function _appendHtml()
    {
        ...
        var checkbox_all = $$("#featured_products_table thead input.checkbox").
            first();
        var everycheckbox = $$("#featured_products_table tbody input.checkbox");
