    ...
    extends Mage_Adminhtml_Controller_Action
{
     ...
     * Method used to display the list of all products to select gift card for baby items.
     */
    public function contentblocktoprightAction()
    {
        ...
        // Set the active menu.
        $this->loadLayout()->_setActiveMenu('zextension/contentblocktopright');

        // Set and load the block content.
        $this->_addContent(
            $this->getLayout()->createBlock('zeon_catalogmanager/adminhtml_Contentblocktopright')
        );
    ...
    /**
     * Method used to save the Content Top Right products
     */
    public function saveContentBlockTopRightAction()
    {
        $data = $this->getRequest()->getPost();
        $storeId = $this->getRequest()->getParam('store', 0);
        parse_str($data['gift_products'], $giftProducts);
        parse_str($data['gift_products_position'], $giftProductsPosition);
        ...
        try {
            if (!empty($giftProducts)) {
                foreach ($giftProducts as $key => $value) {
                    Mage::getSingleton('catalog/product_action')
                        ->updateAttributes(array($key), array('content_top_right'=>$value), $storeId);
                    $indexProducts[] = $key;
                    if (!$value) {
                        Mage::getSingleton('catalog/product_action')
                            ->updateAttributes(array($key), array('content_top_right_position'=>''), $storeId);
                    }
                }
            }
            if (!empty($giftProductsPosition)) {
                foreach ($giftProductsPosition as $key => $value) {
                    ...
                    $product = Mage::getModel('catalog/product')
                        ...
                        ->setStoreId($storeId)
                        ->load($key);
                    $giftForValue = $product->getData('content_top_right')?$product->getData('content_top_right'):Null;
                    if (!in_array($key, $indexProducts)) {
                        $indexProducts[] = $key;
                    }
                    if ($giftForValue && ($value !== NULL)) {
                        Mage::getSingleton('catalog/product_action')
                            ->updateAttributes(array($key), array('content_top_right_position'=>$value), $storeId);
                    }
                }
            }
            if (!empty($indexProducts)) {
            ...
            $this->_getSession()->addSuccess(
                $this->__('Content Top Right Products was successfully saved.')
            );
            $this->_redirect(
