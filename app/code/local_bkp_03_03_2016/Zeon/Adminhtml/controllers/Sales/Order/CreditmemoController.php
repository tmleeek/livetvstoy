...
class Zeon_Adminhtml_Sales_Order_CreditmemoController extends Mage_Adminhtml_Controller_Sales_Creditmemo
{
    /**
    ...
    protected function _saveCreditmemo($creditmemo)
    {
        ...
        $oid= $creditmemo->getOrderId();
       $resource = Mage::getSingleton('core/resource');
       $writeConnection = $resource->getConnection('core_write');
       $query_sel_credit = "SELECT entity_id  FROM sales_flat_creditmemo WHERE 	order_id = '$oid'";
       $results_credit = $writeConnection->fetchAll($query_sel_credit);
       $entity_id= $results_credit[0]['entity_id'];
            
       $query_sel = "SELECT *  FROM vtrio_warehouse_details WHERE 	order_id = '$oid'";
       $results = $writeConnection->fetchAll($query_sel);
       $warehouse='';
       foreach ($results as $a) {
        if($a['warehouse_id']==366)
        $warehouse='Bolingbrook : BX Saleable Location';
        else if($a['warehouse_id']==367)
        $warehouse='Bolingbrook : BX Returns';
        else if($a['warehouse_id']==368)
        $warehouse='Bolingbrook : BX Refund without Return';
        else if($a['warehouse_id']==369)
        $warehouse='Chicago : MBM Refund without Return';
        else if($a['warehouse_id']==370)
        $warehouse='Chicago : MBM Personalized Returns';
        else if($a['warehouse_id']==371)
        $warehouse='Chicago : MBM Stock Return Inspection';
                
        $pId=$a['entity_id'];
        $query_update = "update sales_flat_creditmemo_item set warehouse='$warehouse' WHERE product_id = '$pId' and parent_id='$entity_id'";
