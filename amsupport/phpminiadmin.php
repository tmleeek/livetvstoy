...
*/
 ...
 // Load DB connection settings from Magento
 function load_magento_db_settings() {
    $config = dirname(dirname(__FILE__)) . '/app/etc/local.xml';

    if (! file_exists($config)) {
        return FALSE;
    }
    
    $settings = simplexml_load_file($config, 'SimpleXMLElement', LIBXML_NOCDATA);

    $db = array();
    $db['host']     = (string)$settings->global->resources->default_setup->connection->host;
    $db['username'] = (string)$settings->global->resources->default_setup->connection->username;
    $db['password'] = (string)$settings->global->resources->default_setup->connection->password;
    $db['dbname']   = (string)$settings->global->resources->default_setup->connection->dbname;

    return $db;
 }

 $mage_db = load_magento_db_settings();
 if ($mage_db !== FALSE) {
  $DBDEF=array(
  'user'  => $mage_db['username'],
  'pwd'   => $mage_db['password'],
  'db'    => $mage_db['dbname'],
  'host'  => $mage_db['host'],
  'port'  => '',
  'chset' => 'utf8',
