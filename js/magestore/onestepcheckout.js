...
function add_coupon_code(add_coupon_url) {
    ...
    var request = new Ajax.Request(add_coupon_url, {
        ...
        onSuccess: function (transport) {
            var response = getResponseText(transport);
            if(response.error_form_key){
                alert(response.error_form_key);
            }
            if (response.error) {
                paymentShow();
                reviewShow();
    ...
    var request = new Ajax.Request(url,
        {
            ...
            onSuccess: function (transport) {
                if (transport.status == 200) {
                    ...
                    if (result.url) {
                        //alert("Shopping Cart is Empty. You will be redirect to homepage.");
						document.getElementById('popup-window-cart').style.display = 'block';
                        enablePlaceOrderButton();
                        window.location.href = result.url;
                    } else {
