		...
		switch (this.layout[t][optionId]){
			case 'above' : 
				if (element.type == 'radio' || element.type == 'select-one') {
				  ...
				  this.dd.className ='oc-above-checkbox';					
				}
			break;
			case 'before' : 	
				if (element.type == 'select-one'){
          this.dd.className ='oc-before-select'; 
          Element.wrap(element, 'div', {'class': 'oc-table'});
          Element.wrap(element, 'div', {'class': 'oc-table-cell'});           
          Element.insert(element.up('.oc-table-cell'), {'before': this.makeImage(t, optionId, null, null)});
          Element.wrap($('optionconfigurable_image_' + t + '_' + optionId), 'div', {'class': 'oc-table-cell-img'});						
					Element.insert(element, {'after': new Element('img', {'src' : this.infoIcon, 'class' : 'oc-tooltip-icon', 'title' : '', t: t, id:'optionconfigurable_description_' + t + '_' + optionId, 'style' : 'display:none;'})});									
				} else if(element.type == 'radio'){
          this.dd.className ='oc-before-radio';  
          Element.wrap(this.dd.down('.input-box'), 'div', {'class': 'oc-table'});    
          Element.wrap(this.dd.down('.input-box'), 'div', {'class': 'oc-table-cell'});                    
          Element.insert(this.dd.down('.input-box').up('.oc-table-cell'), {'before': this.makeImage(t, optionId, null, null)});
          Element.wrap($('optionconfigurable_image_' + t + '_' + optionId), 'div', {'class': 'oc-table-cell-img'}); 				
				}	
			break;
			case 'below' :	
				if (element.type == 'radio' || element.type == 'select-one') {	
				  this.dd.className ='oc-below';							
				  ...
				  this.dd.className ='oc-below-checkbox';				
				}
			break;
			case 'swap' : 		
				if (element.type == 'select-one'){					
					this.dd.className ='oc-swap-select';			
					...
					this.dd.className ='oc-swap-radio';						
				}	
			break;
			case 'pickerswap' : 			
			case 'picker' : 				
				this.dd.className ='oc-picker';			
				Element.insert(element, {'after': new Element('div', {'id' : 'optionconfigurable_description_' + t + '_' + optionId, 'style' : 'display:none;'}).addClassName('description')});								
			break;
			case 'grid' : 
				this.dd.className = 'oc-grid';
			...
			break;
			case 'list' : 				
				this.dd.className ='oc-list';				
			break;	
		}	
		
		Element.insert(this.dd, {'bottom': new Element('div').addClassName('oc-note').update(this.note[t][optionId])});
	},
		...
		switch (this.layout[t][optionId]){
			...
			case 'above' : 
				...
				if (value){
					...
					if (this.description[t][valueId] && element.type == 'checkbox'){	
						Element.insert(element.up('li').down('.label'), {'bottom': new Element('img', {'src' : this.infoIcon, 'class' : 'oc-tooltip-icon', 'title' : this.description[t][valueId]})});						
					}						
				}	
				...
				if (value){
					if (this.image[t][valueId]){
						if (element.type == 'radio' || element.type == 'select-one'){			
							...
							if (this.isNewOption){
								Element.insert(this.dd.down('.oc-note'), {'before': this.makeImage(t, optionId, valueId, null)});							
								this.isNewOption = false;								
							} else {	
								Element.insert(previousImage, {'after': this.makeImage(t, optionId, valueId, null)});														
							}
			...
			case 'grid' : 			
				...
				if (value && this.description[t][valueId])
					Element.insert(element, {'after': new Element('img', {'src' : this.infoIcon, 'class' : 'oc-tooltip-icon', 'title' : this.description[t][valueId]})});		
...
<li>
	<label for="options_88_3">
			<img onclick="optionConfigurable.popup(88, 192)" class="oc-image" src="http://...jpg">
	</label>
	<span class="content">
		<label for="options_88_3">
			TITLE HERE ...
...
*/			
	...
	showImage : function(t, optionId, valueId, type){
		if (this.layout[t][optionId] != 'grid' && this.layout[t][optionId] != 'list'){
            ...
            image.setAttribute('oc-data-popup', this.imageDirUrl + this.image[t][valueId]);
					}					
			...
			if (type == 'radio' || type == 'select-one') {
              ...
              this.previousSwapOptions = [];					  
			      ...
			      if (this.popup[t][optionId] && image.style.cursor == 'pointer'){			
				      ...
				      image.title = '';
              image.setAttribute('oc-data-popup', "");
			      }					
    ...
    if (window.jQuery && jQuery.fn.tooltipster){
      jQuery('#optionconfigurable_description_' + t + '_' + optionId).tooltipster('content', this.description[t][valueId]).show();  
    } 	
	},
	...
	highlightPickerImage : function(t, valueId){
		$('optionconfigurable_v_image_' + t + '_' + valueId).className = 'oc-picker-selected';
	},
	
	unhighlightPickerImage : function(t, valueId){
		if (this.image[t][valueId])	
		  $('optionconfigurable_v_image_' + t + '_' + valueId).className = 'oc-picker-unselected';
	},
    ...
    var element,className,onclick,src,popupSrc,style,title,id;	
		switch (this.layout[t][optionId]){
            ...
            popupSrc = this.imageDirUrl + this.image[t][valueId];  										
					}
				} else if (valueId){
			...
			case 'picker' : 	
				...
				className = 'oc-picker-unselected  oc-tooltip';
				src = this.pickerImageDirUrl + this.image[t][valueId];				
				onclick = 'optionConfigurable.reloadSelect(\'' + t + '\', ' + optionId + ', ' + valueId + ')';
        if (this.popup[t][optionId]){
          title = "&lt;img src=&quot;"+this.thumbnailDirUrl + this.image[t][valueId]+"&quot;/&gt;";
        }    							
			break;
		}			
		
		element = '<img src="'+src+'" class="'+className+'"' + (id ? ' id="'+id+'"' : '') + (style ? ' style="'+style+'"' : '') + (title ? ' title="'+title+'"' : '') + (onclick ? ' onclick="'+onclick+'"' : '')  + (popupSrc ? ' oc-data-popup="'+popupSrc+'"' : '') + '/>';							
