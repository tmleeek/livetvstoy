...
OptionConfigurable.Main = Class.create(OptionConfigurable.Images, {
    ...
    this.dependecyIsSet = false;
      ...
      if (Product.ConfigurableSwatches){ // for Magento 1.9.1 
      
        Product.ConfigurableSwatches.prototype.setOptionStatus = function(opt, enabled) {
            var attr = opt.attr;
    ...
    if (oT == 'radio' || oT == 'checkbox') {		
      ...
      if (oT == 'radio'){
        ...
        if (!this.isRequired[t][id] && oT == 'radio'){ //observe None option
          el = $('options_'+id);
          if (el)           
            el.observe('click', this.observeRadio.bind(this, t, id, null));       
        }      
