...
// Function to get Chain details and price
function FUNC_CUSTOM_chains($pid) {
	
	// Get Chain category values from database
	$gift_type  = get_table_info2("product_options po, gift_types gt", "WHERE po.pid = '".$pid."' AND po.optid=gt.id AND gt.placement_step = '' 
					GROUP BY optid ORDER BY gt.output_order", 
					"po.optid optid, gt.title optitle, gt.description description, gt.link_text, gt.show_thumbs, gt.image_place, gt.image_link_text,
					 gt.image image, gt.thumb thumb, gt.link link, gt.required required, gt.display_type display_type, gt.hide hide");
	  
	$gift_cats = get_table_info2("product_options po, gifts g", "WHERE po.pid = '".$pid."' AND po.optid = ".$gift_type[0]["optid"]." 
										AND g.pid = 0
										AND po.optvarid = g.id ORDER BY g.output_order", 
										"po.*, g.title, g.description, g.text_limit, g.image gimage");
	
	 
	// Use Chain categories 
	if(count($gift_cats) > 0) {
		$go_categorized = true;
	}
	  
	foreach($gift_cats as $a => $b) {
		
		// Get Chain item values from database
		$gift_items = get_table_info2("product_options po, gifts g", "WHERE po.pid = '".$pid."' AND po.optid = ".$gift_type[0]["optid"]."
																			AND g.pid = '".$b["optvarid"]."'
																			AND po.optvarid = g.id ORDER BY g.output_order",
																			"po.*, g.title, g.description, g.text_limit, g.image gimage");
		
		// Array of Chain categories
		$arr_cats[] = array('title' 		=> $b['title'],
							'id'			=> $b['id'],
							'optid'			=> $b['optid'],
							'optvarid'		=> $b['optvarid'],
							'group'			=> $a,);
		...
		foreach($gift_items as $a2 => $b2) {
			
			// Get Chain item option values from database
			$gift_items_opts = get_table_info2("product_options po, gifts g", "WHERE po.pid = '".$_GET['pid']."' AND po.optid = ".$gift_type[0]["optid"]."
                                                                        AND g.pid = '".$b2["optvarid"]."'
                                                                        AND po.optvarid = g.id ORDER BY g.output_order",
                                                                        "po.*, g.title, g.description, g.text_limit, g.image gimage");
			
			// Array of Chain item and option values
			$arr_list[] = array('title' 		=> $b2['title'],
								'id'			=> $b2['id'],
								'optid'			=> $b2['optid'],
								'optvarid'		=> $b2['optvarid'],
								'desc'			=> $b2['description'],
								'image'			=> $b2['gimage'],
								'group'			=> $a,
								'options' 		=> $gift_items_opts);
