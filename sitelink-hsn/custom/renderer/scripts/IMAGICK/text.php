...
function FUNC_text_2($layer) {
	...
	$VAR_PT = isset($layer['top']) ? $layer['top'] - $VAR_font_ARR['minus_y'] : 0 - $VAR_font_ARR['minus_y'];
	$VAR_PB = isset($layer['bottom']) ? $layer['bottom'] : 0;
	...
	$VAR_width = $tmp_metrics['textWidth'];
	$VAR_height = $tmp_metrics['textHeight']*count($text_arr)*1.2;
	...
	foreach($text_arr as $key => $val) {
		$tmp_txt_Canvas->annotateImage($draw, 0, $VAR_size*($key), 0, $val);
	}
	...
	if((isset($VAR_rotation) && $VAR_rotation != 0)) {
		...
		if(($VAR_rotation == 90 || $VAR_rotation == -90) && $VAR_PB != 0) {
			$VAR_PT = $core_height - $VAR_PB - $VAR_width;
		}
	}
