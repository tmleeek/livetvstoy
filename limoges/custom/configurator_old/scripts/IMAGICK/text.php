...
function FUNC_text_Arc_i($layer) {
	...
	$text_color = isset($layer['color']) ? $layer['color'] : "black";
	$talpha = isset($layer['talpha']) ? $layer['talpha'] : 1;		// Text alpha
	$text_xoffset = isset($layer['txoffset']) ? $layer['txoffset'] : 0;		// Text horizontal position offset
	$text_yoffset = isset($layer['tyoffset']) ? $layer['tyoffset'] : 0;		// Text vertical position offset
	$text_depth = isset($layer['tdepth']) ? $layer['tdepth'] : 0;		// Text vertical position offset
	...
	if((isset($VAR_fit) && $VAR_fit != 0)) {
		...
		if(!empty($tmp_metrics['textWidth'])) {
	...
	// Draw text on canvas
	if((isset($text_depth) && $text_depth != 0)) {
		// Draw shadow text
		$tx = 0;
		$ty = 0;
	
		for ($i = 1; $i <= $text_depth; $i++) {
			$draw->annotation($tx, $ty, $VAR_txt);
			$tx += $text_xoffset;
			$ty += $text_yoffset;
		}
	}
...
function FUNC_top_text_upper($layer, $engraved, $text_angle) {
	...
	$VAR_color = isset($layer['color']) ? $layer['color'] : "black";
	$VAR_em_color = isset($layer['scolor']) ? $layer['scolor'] : "black";
	$VAR_shadow_color = "black"; // always use black for now
	$VAR_emb_depth = isset($layer['sdepth']) ? $layer['sdepth'] : 5;
	$VAR_side_padding = isset($layer['side_pad']) ? $layer['side_pad'] : 0;
	$VAR_font_size = isset($layer['size']) ? $layer['size'] : 10;
	$VAR_arc_top_rad = isset($layer['arc_top_rad']) ? $layer['arc_top_rad'] : 0;
	...
	if($engraved) {
		$draw->setFillColor($VAR_shadow_color);
	} else {
		$draw->setFillColor($VAR_color);
	}
	...
	$VAR_deg = ($VAR_D / $VAR_txt_width) * $text_angle;
	//$VAR_deg = ($VAR_D / $VAR_txt_width) * 360;
	
	/* Perform the distortion */ 
	$tmp_txt_Canvas->distortImage(Imagick::DISTORTION_ARC, array($text_angle, $VAR_deg, $VAR_arc_top_rad, $VAR_arc_bot_rad), false);
	//$tmp_txt_Canvas->distortImage(Imagick::DISTORTION_ARC, array(360, $VAR_deg, $VAR_arc_top_rad, $VAR_arc_bot_rad), false);
...
function FUNC_top_text_lower($layer, $engraved) {
	...
	$VAR_deg = ($VAR_D / $VAR_txt_width) * 180;
	//$VAR_deg = ($VAR_D / $VAR_txt_width) * 360;
	
	/* Perform the distortion */ 
	$tmp_txt_Canvas->distortImage(Imagick::DISTORTION_ARC, array(180, $VAR_deg, $VAR_arc_top_rad, $VAR_arc_bot_rad), false);
	//$tmp_txt_Canvas->distortImage(Imagick::DISTORTION_ARC, array(360, $VAR_deg, $VAR_arc_top_rad, $VAR_arc_bot_rad), false);
...
function FUNC_text_Wave($layer) {
	...
	$VAR_wave_height = isset($layer['wave_height']) ? $layer['wave_height'] : 0;		// Parameters of wave height and direction
	...
	if((isset($VAR_fit) && $VAR_fit != 0)) {
		if($VAR_length < 3) {
			$tmp_txt_Canvas->newImage($VAR_width + 30, $VAR_new_height + 30, new ImagickPixel("none"));
		} else {
			$tmp_txt_Canvas->newImage($VAR_width + 20, $VAR_new_height + 20, new ImagickPixel("none"));
		}
	} else {
	...
	$tmp_txt_Canvas->waveImage($VAR_wave_height, $tmp_txt_Canvas->getImageWidth());
...
// For Inside Engraving Text
function FUNC_text_Arc_5($layer) {
...
function FUNC_top_text_2($layer) {
...
// Text Engraving New
function FUNC_text_Arc_New_1($layer) {
	...
	// Color Parameters
	$text_color = isset($layer['color']) ? $layer['color'] : "black";
	$text_alpha = isset($layer['talpha']) ? $layer['talpha'] : 1;		// Text alpha
	$text_xoffset = isset($layer['txoffset']) ? $layer['txoffset'] : 0;		// Text horizontal position offset
	$text_yoffset = isset($layer['tyoffset']) ? $layer['tyoffset'] : 0;		// Text vertical position offset
	$text_depth = isset($layer['tdepth']) ? $layer['tdepth'] : 0;		// Text vertical position offset
	...
	// To overwrite metal color
	if($layer['usecolor']) {
		$text_color = isset($layer['ucolor']) ? $layer['ucolor'] : "black";
		$shadow_color = isset($layer['uscolor']) ? $layer['uscolor'] : "black";
	}
	
	// To add stroke color
	if($layer['stroke']) {
		$stroke_color = isset($layer['strokecolor']) ? $layer['strokecolor'] : "white";
		$stroke_width = isset($layer['strokewidth']) ? $layer['strokewidth'] : 1;
	}
	...
	$VAR_xscale = isset($layer['xscale']) ? $layer['xscale'] : 1;	// Horizontal resizing of text canvas
	$VAR_yscale = isset($layer['yscale']) ? $layer['yscale'] : 1;	// Vertical resizing of text canvas
	$VAR_perspective = isset($layer['perspective']) ? $layer['perspective'] : 0;	// Vertical resizing of text canvas
	...
	// Set color and alpha for text
	$draw->setFillColor($text_color);
	$draw->setfillopacity($text_alpha);
	//$draw->setFillAlpha($talpha);
