...
function FUNC_AJD_set_values($layers, $fields, $values = NULL, $W = NULL) {
			...
			foreach ($b['content']['layer'] as $a2 => $b2) {
				...
				foreach ($fields['fields'] as $a3 => $b3) {
					...
					if(in_array($a,$assoc_steps)){
						...
						// if layer has options
						if(isset($b2['option']) && is_array($b2['option'])) {
							
							// For text or wording option
							if(isset($b2['option'][$def_opt]['type']) && $b2['option'][$def_opt]['type'] == 'wording') {
								
								$with_post = false;
								
								if(isset($W) && is_array($W) && count($W) > 0) {
									if(isset($W[$a][$a2]) && is_array($W[$a][$a2])) {
										$with_post = true;
										if($def_val != '') {
											if($def_val != $W[$a][$a2]['NM']) {
												$with_post = false;
												$new_wording = true;
											}
										}
								...
								if($with_post) {
									...
									if (isset($W[$a][$a2]['content']['line']) && is_array($W[$a][$a2]['content']['line'])) {
										foreach($W[$a][$a2]['content']['line'] as &$m){
											
											$m = str_replace("'", "#39", $m);		//replace single quote
											$m = str_replace('"', '#34', $m);		//replace double quote
										}
									}
									
									$AJAX_designer_VARS['product'][$VAR_product_code]['layers'][$a][$a2]['wording'][$a][$a2] = FUNC_wording_re_array($W[$a][$a2], false);
									$AJAX_designer_VARS['product'][$VAR_product_code]['wording'][$a.'-'.$a2][$a][$a2] = $W[$a][$a2];
									
								}else{
									$arr = '';
									$arr = FUNC_get_XML('wording', $def_val, $a2, $a, $def_opt);
									
									// Adjustments to allow single and double quotes in wording
									foreach($arr['content']['line'] as &$k){
										
										$k = str_replace("'", "#39", $k);		//replace single quote
										$k = str_replace('"', '#34', $k);		//replace double quote
									}
									
									
									if(is_array($arr)) {
										$AJAX_designer_VARS['product'][$VAR_product_code]['layers'][$a][$a2]['wording'][$a][$a2] = $arr;
										$AJAX_designer_VARS['product'][$VAR_product_code]['wording'][$a.'-'.$a2][$a][$a2] = FUNC_wording_re_array($arr);		
									}	
								}								
							}
						}
	...
	$AJAX_designer_VARS['mv_itm_rt_F'] = (isset($values['mv_itm_rt_F'])) ? $values['mv_itm_rt_F'] : 0;
	$AJAX_designer_VARS['mv_itm_y_F'] = (isset($values['mv_itm_y_F'])) ? $values['mv_itm_y_F'] : 0;
	$AJAX_designer_VARS['mv_itm_x_F'] = (isset($values['mv_itm_x_F'])) ? $values['mv_itm_x_F'] : 0;
	$AJAX_designer_VARS['mv_itm_rz_F'] = (isset($values['mv_itm_rz_F'])) ? $values['mv_itm_rz_F'] : 0;
	$AJAX_designer_VARS['mv_itm_rt_B'] = (isset($values['mv_itm_rt_B'])) ? $values['mv_itm_rt_B'] : 0;
	$AJAX_designer_VARS['mv_itm_y_B'] = (isset($values['mv_itm_y_B'])) ? $values['mv_itm_y_B'] : 0;
	$AJAX_designer_VARS['mv_itm_x_B'] = (isset($values['mv_itm_x_B'])) ? $values['mv_itm_x_B'] : 0;
	$AJAX_designer_VARS['mv_itm_rz_B'] = (isset($values['mv_itm_rz_B'])) ? $values['mv_itm_rz_B'] : 0;
/*
	$AJAX_designer_VARS['mv_itm_rt'] = $values['mv_itm_rt'];
	$AJAX_designer_VARS['mv_itm_y'] = $values['mv_itm_y'];
	$AJAX_designer_VARS['mv_itm_x'] = $values['mv_itm_x'];
	$AJAX_designer_VARS['mv_itm_rz'] = $values['mv_itm_rz'];
*/
}
